...
class SaleOrder(models.Model):
    ...
    def _compute_amount_multicompany_reporting_currency(self):
        # NOTE: unlike the currency itself, the "total" vs "untaxed" reporting
        # amount option isn't part of the `multicompany.reporting.currency.mixin`
        # API (it is specific to sale orders), so it is stored on our own system
        # parameter instead of depending on a field here; a config change is
        # reflected lazily, see `ResConfigSettings`/`IrConfigParameter` below.
        amount_option = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param(
                "sale_multicompany_reporting_currency.multicompany_reporting_amount",
                "total",
            )
        )
        for record in self:
