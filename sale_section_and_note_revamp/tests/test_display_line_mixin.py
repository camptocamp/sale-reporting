...
class TestDisplayLineMixin(TestDisplayLineMixinCommon):
    ...
    def test_05_get_section_subtotal(self):
        # Calculate the subtotal for the 'price_subtotal' field within the section
        section_subtotal = self.sol_section_1.get_section_subtotal(
            fields=['price_total']
        )
        # Calculate the expected subtotal based on the price units of the lines within the section
    ...
    def test_06_prepare_section_or_note_values(self):
        with self.assertRaises(NotImplementedError):
            self.sol_product_order.prepare_section_or_note_values(
                self.sol_section_1
            )
